...
Expression* ArrayLiteral::RewriteSpreads(Parser* parser) {
  ...
  do_block->statements()->Add(
      parser->factory()->NewExpressionStatement(init_result,
                                                RelocInfo::kNoPosition),
      parser->zone());
  for (int i = first_spread_index_; i < values_->length(); i++) {
    Spread* spread = values_->at(i)->AsSpread();
    if (spread == nullptr) {
      ...
      Variable* element = parser->scope_->NewTemporary(
          parser->ast_value_factory()->dot_result_string());
      // iterator = subject[Symbol.iterator]()
      Expression* assign_iterator = parser->factory()->NewAssignment(
          Token::ASSIGN, parser->factory()->NewVariableProxy(iterator),
          parser->GetIterator(subject, parser->factory(), spread->position()),
          spread->position());
      ...
      Expression* next_element;
      {
        ...
        Expression* iterator_proxy =
            parser->factory()->NewVariableProxy(iterator);
        next_element = parser->BuildIteratorNextResult(iterator_proxy, element,
                                                       spread->position());
      }
      // element.done
      Expression* element_done;
      {
        Expression* done_literal = parser->factory()->NewStringLiteral(
            parser->ast_value_factory()->done_string(), RelocInfo::kNoPosition);
        Expression* element_proxy =
            parser->factory()->NewVariableProxy(element);
        element_done = parser->factory()->NewProperty(
            element_proxy, done_literal, RelocInfo::kNoPosition);
      }
      // each = element.value
      Expression* assign_each;
      {
        Expression* value_literal = parser->factory()->NewStringLiteral(
            parser->ast_value_factory()->value_string(), RelocInfo::kNoPosition);
        Expression* element_proxy =
            parser->factory()->NewVariableProxy(element);
