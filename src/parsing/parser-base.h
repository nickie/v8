...
typename ParserBase<Traits>::ExpressionT ParserBase<Traits>::ParseRegExpLiteral(
  ...
  Next();
  return COST(
      factory()->NewRegExpLiteral(js_pattern, js_flags, literal_index, pos));
}
...
ParserBase<Traits>::ParsePrimaryExpression(ExpressionClassifier* classifier,
  ...
  switch (peek()) {
    ...
    case Token::FALSE_LITERAL:
      BindingPatternUnexpectedToken(classifier);
      return COST(
          this->ExpressionFromLiteral(Next(), beg_pos, scanner(), factory()));
    case Token::SMI:
    case Token::NUMBER:
      BindingPatternUnexpectedToken(classifier);
      return COST(
...
ParserBase<Traits>::ParseYieldExpression(bool accept_IN,
  ...
  typename Traits::Type::YieldExpression yield = COST(factory()->NewYield(
...
ParserBase<Traits>::ParseConditionalExpression(bool accept_IN,
  ...
  ExpressionT expression =
      COST(this->ParseBinaryExpression(4, accept_IN, classifier, CHECK_OK));
...
ParserBase<Traits>::ParseUnaryExpression(ExpressionClassifier* classifier,
  ...
  if (Token::IsUnaryOp(op)) {
    ...
    return COST(factory()->NewCountOperation(op, true /* prefix */, expression,
                                             position()));
...
ParserBase<Traits>::ParsePostfixExpression(ExpressionClassifier* classifier,
  ...
  if (!scanner()->HasAnyLineTerminatorBeforeNext() &&
    ...
    Token::Value next = Next();
    expression = COST(factory()->NewCountOperation(next, false /* postfix */,
                                                   expression, position()));
  }
  return expression;
}
  ...
  while (true) {
    switch (peek()) {
      ...
      case Token::LPAREN: {
        ...
        if (is_super_call) {
          ExpressionT this_expr =
              COST(this->ThisExpression(scope(), factory(), pos));
          result = COST(
      ...
      case Token::TEMPLATE_TAIL: {
        ...
        ArrowFormalParametersUnexpectedToken(classifier);
        result = COST(
            ParseTemplateLiteral(result, position(), classifier, CHECK_OK));
        break;
      }
...
ParserBase<Traits>::ParseMemberWithNewPrefixesExpression(
  ...
  if (peek() == Token::NEW) {
    ...
    return COST(factory()->NewCallNew(result, this->NewExpressionList(0, zone_),
                                      new_pos));
  }
...
ParserBase<Traits>::ParseMemberExpression(ExpressionClassifier* classifier,
  ...
  result = COST(ParseMemberExpressionContinuation(result, is_async, classifier,
                                                  CHECK_OK));
  return result;
}
...
ParserBase<Traits>::ParseSuperExpression(bool is_new,
  ...
  if (IsConciseMethod(kind) || IsAccessorFunction(kind) ||
    ...
    if (peek() == Token::PERIOD || peek() == Token::LBRACK) {
      ...
      return COST(
          this->NewSuperPropertyReference(this->scope(), factory(), pos));
    }
    ...
    switch (peek()) {
      ...
      case Token::TEMPLATE_TAIL: {
        *is_async = false;
        ...
        expression =
            COST(ParseTemplateLiteral(expression, pos, classifier, CHECK_OK));
        break;
      }
      case Token::ILLEGAL: {
